...
export default asyncHandler(async function isUserLoginedmiddleware(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  let isUserLogined;
  try {
    isUserLogined = await UserModel.findById(userId);
  } catch (error: any) {
    return next(
      res
        .status(500)
  ...
  const user = isUserLogined?._id;

  if (!user) {
    return res
      .status(403)
      .json(ApiResponse(403, "User not found.", null, null));
  }
  return next();
});
